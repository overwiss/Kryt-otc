...
admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="💰 Фейк пополнение (Баланс)", callback_data="send_money")],
    [InlineKeyboardButton(text="✅ Успешные сделки", callback_data="set_successful_deals")],
    [InlineKeyboardButton(text="📊 Всего сделок", callback_data="set_total_deals")],
...
async def admin_panel_command(message: Message):
    # Проверка: только админ может открыть панель
    if message.from_user.id != ADMIN_ID:
        return # Бот просто проигнорирует не-админа

    await message.answer(
        "👑 <b>Админ-панель FunPay OTC</b>\n\n"
        "Выберите действие для управления ботом или статистикой:",
...
async def start_command(message: Message):
    # ... (весь твой текущий код старта без изменений)
...
async def deal_type_callback(callback: CallbackQuery):
    ...
    if lang == "ru":
        await safe_edit_message(
            ...
            "Например: <i>Discord Nitro 1 Month / Telegram Premium</i>",
            back_keyboard_ru
        )
    else:
        await safe_edit_message(
            ...
            "Example: <i>Discord Nitro 1 Month / Telegram Premium</i>",
            back_keyboard_en
        )

# Добавляем недостающий обработчик выбора валюты
@dp.callback_query(F.data.startswith("currency_"))
async def currency_select_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    if user_id not in user_deals:
        return
    ...
    lang = user_languages.get(user_id, "ru")
    if lang == "ru":
        await safe_edit_message(callback, f"💰 <b>Введите сумму сделки в {currency}:</b>")
    else:
        await safe_edit_message(callback, f"💰 <b>Enter deal amount in {currency}:</b>")
...
async def send_money_callback(callback: CallbackQuery):
    ...
    await callback.message.answer("💰 <b>Введите ID пользователя и сумму для зачисления</b>\n\nФормат: <code>12345678 500</code>")
